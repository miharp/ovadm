...
The agent's output names who compiled its catalog (`Catalog compiled by
compiler01…`). That line is what shows the compiler is doing the work rather
than the server behind it, and `.github/workflows/install-test.yml` checks for it
