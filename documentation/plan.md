...
4. ~~**OpenVoxDB**~~ — Confirmed: package names are `openvoxdb` and `openvoxdb-termini`; service name is `puppetdb`. Requires PostgreSQL 14+ with the `pg_trgm` extension. Listens on HTTP port 8080 (localhost only, no auth) and HTTPS port 8081 (mutual TLS, requires client cert). Not required for a basic install. An `ovadm::add_openvoxdb` plan was prototyped and tested (PR #32) but closed — the PostgreSQL prerequisite has too many infrastructure-specific variables (cloud RDS, managed services, auth config) to own well in a Bolt plan, and most operators will manage OpenVoxDB via Puppet modules instead.
