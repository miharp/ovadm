...
describe 'ovadm::codavox' do
  ...
  before(:each) do
    ...
    allow_task('ovadm::install_codavox').always_return('status' => 'success', 'version' => '0.6.2')
  ...
  it 'installs, configures both roles, serves, converges, then wires the compilers' do
    expect_task('ovadm::install_codavox')
      .always_return('status' => 'success', 'version' => '0.6.2')
      .be_called_times(1)
  ...
  it 'fails when the installed codavox is not the one requested' do
    ...
    result = run_plan('ovadm::codavox', {
      'server_host'    => server,
      'compiler_hosts' => compiler,
      'codavox_version' => '0.6.2'
    })
    expect(result).not_to be_ok
    expect(result.value.msg).to match(%r{has codavox 0\.4\.0, but 0\.6\.2 was requested})
  end
